$(function () {
    ...
    var strhtml_searchContent = '<div class="inline-block margin">'
        + '<span>充值用户手机号:</span>'
        + '<input type="text" class="inputStyle_condition rechargemobile"/>'
        + '<span>佣金用户手机号:</span>'
    ...
    $("body").delegate('.detailit', 'click', function () {
        layer.open({
            ...
            content: '../record/corecommissionrecord_detail.html?id=' + $(this).attr("data-id") + '&timestamp=' + (new Date()).valueOf()
        });
    });
    //全选
...
function showList(obj, pagenum) {
    var aData = [{name: "<input type='checkbox' name='checkboxAll' value='checkbox' />", percent: "5"},
        {name: "充值用户手机号", percent: "15"},
        {name: "充值金额", percent: "10"},
        {name: "充值时间", percent: "15"},
        {name: "佣金用户手机号", percent: "15"},
        {name: "佣金金额", percent: "10"},
        {name: "佣金比", percent: "7"},
        {name: "分销等级", percent: "8"},
        {name: "操作", percent: "15"}];
    setTableHead(aData);
    var str = 'pageNum=' + pagenum + '&pageSize=15';
    if (obj.rechargemobile != "") {
        str += '&rechargemobile=' + encodeURIComponent(obj.rechargemobile);
    }
    if (obj.mobile != "") {
        str += '&mobile=' + encodeURIComponent(obj.mobile);
    }
    getOData(str, "coreCommissionRecord/find/by/cnd", {
            fn: function (oData) {
                var strhtml_list = "";
                ...
                for (var i = 0; i < ln; i++) {
                    strhtml_list += '<tr class="trHighLight">'
                        + '<td>' + '<input type="checkbox" name="checkbox" value="checkbox" data-id="' + arrData[i].crcrdUuid + '"/>' + '</td>'
                        + '<td>' + (arrData[i].crcrdUserMobile || "") + '</td>'
                        + '<td>' + (arrData[i].crcrdFee || 0) + '</td>'
                        + '<td>' + (getFormatDate(arrData[i].crcrdDate) || "") + '</td>'
                        + '<td>' + (arrData[i].crcrdBeUserMobile || "") + '</td>'
                        + '<td>' + (arrData[i].crcrdBeFee || 0) + '</td>'
                        + '<td>' + (arrData[i].crcrdRatio || 0) + '</td>'
                        + '<td>' + (arrData[i].crcrdLevel==1?"一级":"二级") + '</td>'
                        + '<td>'
                        + '<a  class="p-edit detailit" data-id="' + arrData[i].crcrdUuid + '">查看</a>'
                        + '</td>'
                        + '</tr>';
                }
...
function searchContent(obj) {
    obj.rechargemobile = $(".rechargemobile").val();
