...
public abstract class DataRequiredReflect {
	...
	protected Object[] getNeedDetailIds(List list, Class dataClass,Class needDetailed, List<String[]> detailMethodNames) {
		...
		try {
			...
			for (String[] getSetName : detailMethodNames) {
				Method getMethod = dataClass.getMethod(getSetName[0]);
				if (getMethod != null)
			...
			Method getIdMethod = needDetailed.getMethod("getId");
			for (Object obj : list) {
				if(obj==null){
					continue;
				}
				for (Method method : getMethods) {
					Object detailedObj = method.invoke(obj);
					if (detailedObj != null) {
	...
	protected String[] getNeedDetailStringIds(List list, Class dataClass,
		...
		try {
			List<Method> getMethods = new ArrayList<Method>();
			for (String[] getSetName : detailMethodNames) {
				Method setMethod = dataClass.getMethod(getSetName[0]);
				if (setMethod != null)
			...
			if (getIdMethod != null)
				for (Object obj : list) {
					for (Method method : getMethods) {
						Object propertyVal = method.invoke(obj);
						if (propertyVal != null) {
				...
				for (Object obj : list) {
					...
					for (String[] getSetName : detailMethodNames) {
						...
						if (getMethod != null && setMethod != null) {
							...
							Object detailedObj = getMethod.invoke(obj);
